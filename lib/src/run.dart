...
class ScriptRunner {
  ...
  Future<CompileResult> compileAot(String outputPath, {bool verbose = false}) async {
    final mainDart = path.join(workingDirectory, 'bin', 'main.dart');
    if (!File(mainDart).existsSync()) {
  ...
  Future<RunResult> runAot(String aotPath, {
    List<String> args = const [],
    bool verbose = false,
  }) async {
    ...
    if (runCommand.length == 1) {
      ...
      process = await Process.start(runCommand.first, [...runCommand.skip(1), ...runArgs]);
    }
